...
switch ($_GET["op"]){
    ...
    case 'listar':
        ...
        while ($reg=$rspta->fetch_object()){
            $data[] = array(
                            "0" =>'
                            <button class="btn btn-danger btn-sm" onclick="eliminar('.$reg->idpagoproveedor.')"><i class="fa fa-times" aria-hidden="true"></i></button>
                            <button class="btn btn-warning btn-sm" onclick="mostrar('.$reg->idpagoproveedor.')"><i class="fas fa-edit"></i></button>',
                            "1" => $reg->fecha,
                            "2" => $reg->concepto,
